...
mntodfa/REGEX-B/61.25/5.375/11 cm/Myhill-Nerode relatie naar DFA/$\mnrel\rightarrow\mbox{DFA}$/{Gegeven een taal $L$ over een alfabet $\Sigma$ en een MN-relatie $\sim$ over $\Sigma^*$. Dan is $\tupl{Q,\Sigma,\delta,q_s,F}$ een DFA die $L$ bepaalt met: $Q=\accl{x_{\sim}|x\in\Sigma}$, $q_s=\epsilon_{\sim}$, $F=\accl{x_{\sim}|x\in L}$ en $\fun{\delta}{x_{\sim},a}=\brak{xa}_{\sim}$.},
