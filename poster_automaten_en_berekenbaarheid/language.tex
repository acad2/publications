...
ift/LAMBDA/53.5/1.5/8 cm/{Lijsten}/$\vec{x}$/{Lijsten worden meestal door het gelinkte-lijst paradigma voorgesteld. Elke node heeft een \textsc{Hoofd} en een \textsc{Staart}. Men stelt een node voor met \textsc{Const}:\begin{itemize}\item $\mbox{\sc Hoofd}=\lambda\ x\ y\ .\ x$\item $\mbox{\bf false}=\lambda\ x\ y\ .\ y$\item $\mbox{\bf if}=\lambda\ x\ y\ z.\brak{x\ y\ z}$\end{itemize}},
