...
\begin{defi}[Ewff defining a predicate]
The \term{Ewff defining a predicate} denoted $e_P$ is the disjunction of all $e$ such that $\forall\overline{x}\brak{\fun{e}{\overline{x}}\supset\fun{P}{\overline{x}}}\in\Sigma$. The ewff defining $\neg P$ denoted $e_{\neg P}$ is analogous.
\cite{conf/ijcai/LiuL03}
\end{defi}

\begin{defi}[Relational ewff, Co-relational ewff, Unrestricted ewff]
We distinguish among three cases for an ewff:
\begin{enumerate}
 \item $e$ is \term[Relational ewff]{relational} if it is of the form: $\overline{x}=\overline{e_1}\vee\overline{x}=\overline{e_2}\vee\ldots\vee\overline{x}=\overline{e_n}$ where $n\geq0$.
 \item $e$ is \term[Co-relational ewff]{co-relational} if it is of the form: $\overline{x}\neq\overline{e_1}\vee\overline{x}\neq\overline{e_2}\vee\ldots\vee\overline{x}\neq\overline{e_n}$ where $n\geq0$.
 \item $e$ is \term[Unrestricted ewff]{unrestricted} otherwise.
\end{enumerate}
\cite{conf/ijcai/LiuL03}
\end{defi}

\begin{defi}[$e$-size a knowledge base]
The \term[$e$-size a knowledge base]{$e$-size of $\Sigma$} is the maximum size of an unrestricted $e_P$ or $e_{\neg P}$ in $\Sigma$.
\cite{conf/ijcai/LiuL03}
\end{defi}

\begin{defi}[$k$-guarded fragment of a proper knowledge base]
Let $\Sigma$ be a proper knowledge base. The \term[$k$-guarded fragment of a proper knowledge base]{strictly $k$-guarded fragment \fun{SG_k}{\Sigma}} of $\calL$ for $\Sigma$ is the same as $SGF_k$ except that when $\exists\overline{x}\brak{G_1\wedge G_2\wedge\ldots\wedge G_m\wedge\phi}$ is withing scope of an even (\respTx{} odd) number of $\neg s$, the $G_i$ must be equalities or atoms \fun{P}{\overline{t}}, \stTx{} $c_P$ is relational (\respTx{} $e_{\neg P}$ is co-relational) or literals $\neg\fun{P}{\overline{t}}$ \stTx{} $e_{\neg P}$ is relational (\respTx{} $e_P$ is co-relational).
\cite{conf/ijcai/LiuL03}
\end{defi}

\begin{defi}[Conditional rewriting]
Let $R$ be a set of conditional equations. Let $a$ be a term and $u$ a position in $a$. We write: \term[Conditional rewriting]{$\funf{a}{s\sigma}_u\rightarrow_R\funf{a}{t\sigma}_u$} if there is a substitution $\sigma$ and a conditional equation $\displaystyle\wedge_{i=0}^n a_i=b_i\Rightarrow s=t$ in $R$ such that:
\begin{enumerate}
...
\begin{defi}[Completely defined operator \wrtTx{} a set of constructors]
Let $C$ be a set of constructors, let $R$ be a rewrite system and $D$ be a set of defined operators. The operator $f\in D$ is \term[Completely defined operator with respect to a set of constructors]{completely defined \wrtTx{} $C$} if \faTx{} $t_1,t_2,\ldots,t_n$ in \fun{T}{C} \teTx{} $t$ in \fun{T}{C} such that $\fun{f}{t_1,t_2,\ldots,t_n}\rightarrow_R^{\star} t$.
\cite{conf/ijcai/BouhoulaR93}
\end{defi}

\begin{defi}[Inductive rewriting]
Let $R$ and $W$ be two sets of conditional equations. Consider a clause $C\equiv\brak{a=b}^{\epsilon}\vee r$ and its Skolemized version $\overline{C}\equiv\brak{\overline{a}=\overline{b}}^{\epsilon}\vee\overline{r}$. We write: \term[Inductive rewriting]{$a\rightarrow_{\funf{R}{W};r}a'$} if $a\succ a'$ and: either $\overline{a}\rightarrow_{\funm{prem}{\overline{r}}}\overline{a'}$, or \teTx{} a position $u$ in $a$, a substitution $\sigma$ and a conditional equation $\displaystyle\wedge_{i=1}^n a_i=b_i\Rightarrow s=t$ in $R$ \stTx{}:
\begin{enumerate}
...
\end{enumerate}
when $=_{\funm{prem}{\overline{r}}}$ is the congruence generated by \funm{prem}{\overline{r}}.
\cite{conf/ijcai/BouhoulaR93}
\end{defi}

\begin{defi}[Inductive positions for a function symbol]
Given a rewrite $R$ on \fun{T}{P,X} the set of \term[Inductive positions for a function symbol]{inductive positions for a function symbol $f$} is: $\funm{Occ\_ind}{R,f}=\accol{u/\mbox{ \teTx{}} p\Rightarrow g \rightarrow d\in R\mbox{ \stTx{} }g\in\funm{Def}{f},u\in\funm{Occ}{g}\setminus\accol{\epsilon}\mbox{ and }g/u\notin\funm{VL}{g}}$.
\cite{conf/ijcai/BouhoulaR93}
\end{defi}

\begin{defi}[Quasi-inconsistent \wrtTx{} a rewrite system]
Suppose that we are given a rewrite system $R$ and a test set \fun{S}{R}. Then a clause $C\equiv g_1=d_1\vee g_2=d_2\vee\ldots\vee g_n=d_n$ is \term[Quasi-inconsistent \wrtTx{} a rewrite system]{quasi-inconsistent \wrtTx{} $R$} if \tiTx{} a \fun{S}{R}-substitution $\sigma$ of $C$ \stTx{} \faTx{} $1\leq j\leq n$, $g_j\sigma\not\equiv d_j\sigma$: and the maximal elements of \accol{g_j\sigma,d_j\sigma} \wrtTx{} $\succ$ are strongly R-reducible.
\cite{conf/ijcai/BouhoulaR93}
\end{defi}

\begin{defi}[Case rewriting]
Let $R$ be a rewrite system and $C\equiv\brak{a=b}^{\epsilon}\vee r$ be a clause. We define $G$ as the set $\accol{\tuple{\funf{a}{d\sigma}_u,P\sigma}; \mbox{\teTx{} }P\Rightarrow g\rightarrow d\mbox{ in}R,\mbox{a position }u\mbox{ in }a\mbox{ \stTx{} }a/u=g\sigma, g\sigma\mbox{ is R-irreducible and does not contain an inductive variable}}$. Then \funm{Case\_rewriting}{\brak{a=b}^{\epsilon},r} is the following set of clauses:
\begin{equation}
...
\begin{defi}[Separated union of $r$-descriptions]
For a pair of $r$-descriptions $D_1=\accol{\tuple{\varphi_1,\Delta_1},\tuple{\varphi_2,\Delta_2},\ldots,\tuple{\varphi_k,\Delta_k}}$ and $D_2=\accol{\tuple{\eta_1,\Theta_1},\tuple{\eta_2,\Theta_2},\ldots,\tuple{\eta_h,\Theta_h}}$, the \term[Separated union of $r$-descriptions]{separated union $D_1\oplus D_2$} of $D_1$ and $D_2$ is the smallest $r$-description such that for each atomic $r$-description $\tuple{\varphi_i,\Delta_i}\in D_1$ and each atomic $r$-description $\tuple{\eta_j,\Theta_j}\in D_2$:
\begin{enumerate}
...
\begin{defi}[Relation quasi-commutes over another relation]
For a pair of relations $<_1$ and $<_2$ on some set $S$. \term[Relation quasi-commutes over another relation]{$<_1$ quasi-commutes over $<_2$} \iffTx{} \faTx{} $q,t,r\in S$ with $q<_2t<_1r$ some $t'\in S$ exists such that $q<_{1\cup2}^{\star}t'<_2r$ (see figure \ref{fig:quasiCommutesRelation}), where $<_{1\cup2}^{\star}$ denotes the reflexive and transitive closure of $<_1\cup<_2$.
\cite{conf/ijcai/Walther93}
\end{defi}

\begin{figure}[hbt]
\centering
\subfigure[Quasi-commutation]{\begin{tikzpicture}
\node (q) at (0,3) {$q$};
\node (t2) at (3,3) {$t'$};
\node (t) at (0,0) {$t$};
\node (r) at (3,0) {$r$};
\draw[->] (r) to node[midway,below] {$<_1$} (t);
\draw[dashed,->] (r) to node[midway,right] {$<_2$} (t2);
\draw[->] (t) to node[midway,left] {$<_2$} (q);
\draw[dashed,->] (t2) to node[midway,above] {$\brak{<_1\cup<_2}^{\star}$} (q);
\end{tikzpicture}\label{fig:quasiCommutesRelation}}
\subfigure[$\Lambda$-commutation]{\begin{tikzpicture}
\node (q) at (0,3) {$q$};
\node (t2) at (3,3) {$t'$};
\node (t) at (0,0) {$t$};
\node (r) at (3,0) {$r$};
...
\draw[dashed,->] (r) to node[midway,right] {$<_2$} (t2);
\draw[->] (t) to node[midway,left] {$<_2$} (q);
\draw[dashed,->] (t2) to node[midway,above] {$<_{\Lambda}$} (q);
\end{tikzpicture}\label{fig:lambdaCommutesRelation}}
\subfigure[$\varepsilon$-commutation]{\begin{tikzpicture}
\node (q) at (0,3) {$q$};
\node (t) at (0,0) {$t$};
\node (r) at (3,0) {$r$};
\draw[->] (r) to node[midway,below] {$<_1$} (t);
\draw[->] (t) to node[midway,left] {$<_2$} (q);
\draw[dashed,->] (r) to node[midway,above] {$<_2$} (q);
\end{tikzpicture}\label{fig:epsilonCommutesRelation}}
\subfigure[$1$-commutation]{\begin{tikzpicture}
\node (q) at (0,3) {$q$};
\node (t2) at (3,3) {$t'$};
\node (t) at (0,0) {$t$};
\node (r) at (3,0) {$r$};
...
\draw[dashed,->] (r) to node[midway,right] {$<_2$} (t2);
\draw[->] (t) to node[midway,left] {$<_2$} (q);
\draw[dashed,->] (t2) to node[midway,above] {$<_1$} (q);
\end{tikzpicture}\label{fig:oneCommutesRelation}}
\caption{Different types of quasi-commutation.}
\end{figure}

\begin{defi}[Relation $\Lambda$-commutes over another relation]
For a pair of relations $<_1$ and $<_2$ on some set $S$. \term[Relation $\Lambda$-commutes over another relation]{$<_1$ quasi-commutes over $<_2$ with type $\Lambda$} \iffTx{} \faTx{} $q,t,r\in S$ with $q<_2t<_1r$ some $t'\in S$ exists such that $q<_{\Lambda}t'<_2r$, where $\Lambda$ is some finite string over \accol{1,2} (see figure \ref{fig:lambdaCommutesRelation}), $a<_{\Lambda\circ d}b$ stands for ``some $c\in S$ exists \stTx{} $a<_{\Lambda}c<_db$'' and $a<_{\varepsilon}b$ means $a = b$ (where $\varepsilon$ is the empty string, see figure \ref{fig:epsilonCommutesRelation}).
\cite{conf/ijcai/Walther93}
\end{defi}

\begin{defi}[$r$-description quasi-commutes over another $r$-description with type $1$]
For a pair of $r$-descriptions $D_1=\accol{\tuple{\varphi,\accol{\delta}}}$ and $D_1=\accol{\tuple{\eta,\accol{\theta}}}$, \term[$r$-description quasi-commutes over another $r$-description with type $1$]{$D_1$ quasi-commutes over $D_2$ with type $1$} \tabbrev{$r$-description quasi-commutes over another $r$-description with type $1$}{$1$-commutes} \iffTx{} \fsTx{} variables $x^{\star}$:
\begin{enumerate}
 \item $\fun{rV}{D_1}=\funm{Dom}{\delta}=\fun{rV}{D_2}=\funm{Dom}{\theta}=x^{\star}$,
 \item $\flatbrak{\forall x^{\star}.\varphi\wedge\fun{\delta}{\eta}\rightarrow\eta\wedge\fun{\theta}{\varphi}}\in\mbox{Th}_{\mbox{Ind}}$, and
 \item $\flatbrak{\forall x^{\star}.\varphi\wedge\fun{\delta}{\eta}\rightarrow\displaystyle\wedge_{x\in x^{\star}}\fun{\theta}{\fun{\delta}{x}}=\fun{\delta}{\fun{\theta}{x}}}\in\mbox{Th}_{\mbox{Ind}}$.
\end{enumerate}
\cite{conf/ijcai/Walther93}
\end{defi}

\begin{defi}[$r$-description quasi-commutes over another $r$-description with type $\varepsilon$]
For a pair of $r$-descriptions $D_1=\accol{\tuple{\varphi,\accol{\delta}}}$ and $D_1=\accol{\tuple{\eta,\accol{\theta}}}$, \term[$r$-description quasi-commutes over another $r$-description with type $\varepsilon$]{$D_1$ quasi-commutes over $D_2$ with type $\varepsilon$} \tabbrev{$r$-description quasi-commutes over another $r$-description with type $\varepsilon$}{$\varepsilon$-commutes} \iffTx{} for $x^{\star}=\fun{rV}{D_1}=\fun{rV}{D_2}$:
\begin{enumerate}
 \item $\mbox{all variables in }\eta\subseteq\funm{Dom}{\delta}$,
 \item $\flatbrak{\forall x^{\star}.\varphi\wedge\fun{\delta}{\eta}\rightarrow\eta}\in\mbox{Th}_{\mbox{Ind}}$, and
 \item $\flatbrak{\forall x^{\star}.\varphi\wedge\fun{\delta}{\eta}\rightarrow\displaystyle\wedge_{x\in\funm{Dom}{\theta}}\fun{\theta}{\fun{\delta}{x}}=\fun{\theta}{x}}\in\mbox{Th}_{\mbox{Ind}}$.
\end{enumerate}
\cite{conf/ijcai/Walther93}
\end{defi}

\begin{defi}[Order relation-closure of a program]
Given a program $\calP=\tuple{\Pi,\calN,<}$. \fun{\calP}{<^+} is the \term[Order relation-closure of a program]{$<$-closure of $\calP$} \iffTx{} \fun{\calP}{<^+} is the smallest set containing \fun{\calP}{<} and closed under transitivity.
\cite{conf/ijcai/ZhangF97}
\end{defi}

\begin{defi}[Rule defeated by an extended logic program]
Let $\Pi$ be an extended logic program and $r$ a rule with the form $L_0\leftarrow L_1,L_2,\ldots L_m,\neg L_{m+1},\neg L_{m+2},\ldots,\neg L_n$ ($r$ does not necessarily belongs to $\Pi$). \term[Rule defeated by an extended logic program]{Rule $r$ is defeated by $\Pi$} \iffTx{} \fanTx{} answer set \funm{Ans}{\Pi} of $\Pi$, \teTx{} some $L_i\in\funm{Ans}{\Pi}$, where $m+1\leq i\leq n$.
\cite{conf/ijcai/ZhangF97}
\end{defi}

\begin{defi}[Reduct of a prioritized extended logic program with respect to an order relation]
Let $\calP=\tuple{\Pi,\calN,<}$ be a prioritized extended logic program. We define a \term[Reduct of a prioritized extended logic program with respect to an order relation]{reduct of $\calP$ with respect to $<$}, denoted as $\calP^{<}$, as follows:
\begin{enumerate}
 \item $\Pi_0=\Pi$,
...
\begin{defi}[Answer set of a prioritized logic program]
Let $\calP=\tuple{\Pi,\calN,<}$ be a prioritized logic program and $\mbox{Lit}$ the set of all ground literals in the language of $\calP$. \fanTx{} subset $S$ of $\mbox{Lit}$, $S$ is an \term[Answer set of a prioritized logic program]{answer set of $\calP$}, denoted as $\fun{\mbox{Ans}^P}{\calP}$, \iffTx{} $S=\funm{Ans}{\calP^<}$.
\cite{conf/ijcai/ZhangF97}
\end{defi}

\begin{defi}[Concrete domain, Domain of a concrete domain, Admissible concrete domain]
A \term{concrete domain} $\calD$ is a pair \tuple{\Delta_{\calD},\Phi_{\calD}} where $\Delta_{\calD}$ is a set called the \term[Domain of the concrete domain]{domain}, and $\Phi_{\calD}$ is a set of predicate names. Each predicate $P$ in $\Phi_{\calD}$ is associated with an arity $n$ and an $n$-ary predicate $P^{\calD}\subseteq\Delta_{\calD}^n$. A concrete domain is called \term[Admissible concrete domain]{admissible} \iffTx{}:
\begin{enumerate}
...
\begin{defi}[Feature chain, Concept, Negation as concept, Conjunction as concept, Disjunction as concept, Value restriction as concept, Exists restriction as concept, Predicate operator as concept, Feature agreement as concept, Feature disagreement as concept]
Let $C$, $R$, and $F$ be disjoint sets of concept, role, and feature names. A composition $f_1f_2\ldots f_n$ of features is called a \term{feature chain}. Any element of $C$ is a \term{concept}. If $C$ and $D$ are concepts, $R$ is a role or feature, $P\in\Phi_{\calD}$ is a predicate name with arity $n$, and $u_1,u_2,\ldots,u_n$ are feature chains, then the following expressions are also concepts:
\begin{enumerate}
 \item $\neg C$ (\term[Negation as concept]{negation}), $C\sqcap D$(\term[Conjunction as concept]{conjunction}), $C\sqcup D$ (\term[Disjunction as concept]{disjunction}), $\forall R.C$ (\term[Value restriction as concept]{value restriction}), $\exists R.C$ (\term[Exists restriction as concept]{exists restriction})
 \item $\exists u_1,u_2,\ldots,u_n.P$ (\term[Predicate operator as concept]{predicate operator})
 \item $u_1\downarrow u_2$ (\term[Feature agreement as concept]{feature agreement}), $u_1\uparrow u_2$ (\term[Feature disagreement as concept]{feature disagreement})
\end{enumerate}
\label{def:featurechain}
\cite{conf/ijcai/Lutz99}
\end{defi}

\begin{defi}[Interpretation, Abstract domain of an interpretation, Interpretation function, Model of a concept, Satisfiable concept, Concept subsumes other concept]
An \term{interpretation} $\calI$ consists of a set $\Delta_{\calI}$ (the \term[Abstract domain of an interpretation]{abstract domain}) and an \term{interpretation function} $.^{\calI}$. The sets $\Delta_{\calD}$ and $\Delta_{\calD}$ must be disjoint. The interpretation function maps each concept name $C$ to a subset $C^{\calI}$ of $\Delta_{\calI}$, each role name $R$ to a subset $R^{\calI}$ of $\Delta_{\calI}\times\Delta_{\calI}$, and each feature name $f$ to a partial function $f^{\calI}$ from $\Delta_{\calI}$ to $\Delta_{\calD}\cup\Delta_{\calI}$, where $\fun{f^{\calI}}{a}=x$ will be written as $\tuple{a,x}\in f^{\calI}$. If $u=f_1f_2\ldots f_k$ is a feature chain, then $u^{\calI}$ is defined as the composition $f_1^{\calI}\circ f_2^{\calI}\circ\ldots f_k^{\calI}$ of the partial functions $f_1^{\calI},f_2^{\calI},\ldots,f_k^{\calI}$. Let the symbols $C$, $D$, $R$, $P$, and $u_1,u_2,\ldots u_n$ be defined as in \defref{featurechain}. Then the interpretation function can be extended to complex concepts as 
follows:
\begin{align}
...
\end{align}
An interpretation $\calI$ is a \term{model of a concept} $C$ \iffTx{} $C^{\calI}\neq\emptyset$. A concept $C$ is \term[Satisfiable concept]{satisfiable} \iffTx{} there exists a model $\calI$ of $C$. A concept $C$ \term[Concept subsumes other concept]{subsumes} a concept $D$ (written $D\preceq C$) \iffTx{} $D^{\calI}\subseteq C^{\calI}$ for all interpretations $\calI$.
\cite{conf/ijcai/Lutz99}
\end{defi}

\begin{defi}[Concrete objects, Abstract objects, Assertional axioms, ABox, Interpretation satisfies an assertional axiom, Model of an ABox, Consistent ABox]
Let $O_D$ and $O_A$ be disjoint sets of object names. Elements of $O_D$ are called \term{concrete objects} and elements of $O_A$ are called \term{abstract objects}. If $C$, $R$, $f$, and $P$ are defined as in \defref{featurechain}, $a$ and $b$ are elements of $O_A$ and $x_1,x_2,\ldots,x_n$ are elements of $O_D$, then the following expressions are \term{assertional axioms}:
\begin{equation}
a:C,\tuple{a,b}:R,\tuple{a,b}:f,a\neq b,\tuple{x_1,x_2,\ldots,x_n}:P
\end{equation}
A finite set of assertional axioms is called an $\fun{\calA\calL\calC\calF}{\calD}$ \term{ABox}. An interpretation for the concept language can be extended to the assertional language by mapping every object name from $O_A$ to an element of $\Delta_{\calI}$ and every object name from $O_D$ to an element of $\Delta_{\calD}$. The unique name assumption is not imposed, i.e. $a^{\calI}=b^{\calI}$ may hold even if $a$ and $b$ are distinct object names. An \term{interpretation satisfies an assertional axiom}
\begin{align}
...
\end{align}
An interpretation is a \term{model of an ABox} $\calA$ \iffTx{} it satisfies all assertional axioms in $\calA$. An ABox is \term[Consistent ABox]{consistent} \iffTx{} it has a model.
\cite{conf/ijcai/Lutz99}
\end{defi}

\begin{defi}[Completion rules, ABox containing a fork, Conjugate rule, (Nondeterministic) disjunction rule, Role exists restriction rule, Feature exists restriction rule, Role value restriction rule, Feature value restriction rule, Predicate exists restriction rule, Agreement rule, Disagreement rule, Generating rule applications, Non-generating rule applications]
The following \term{completion rules} replace a given ABox $\calA$ nondeterministically by an ABox $\calA'$. An ABox $\calA$ is said to \term[ABox containing a fork]{contain a fork} (for a feature $f$) \iffTx{} it contains the two axioms $\tuple{a,b}:f$ and $\tuple{a,c}:f$ or the two axioms $\tuple{a,x}:f$ and $\tuple{a,y}:f$ where $b,c\in O_A$ and $x,y\in O_D$. A fork can be eliminated by replacing all occurrences of $c$ in $\calA$ with $b$, or of $x$ with $y$, resp. It is assumed that forks are eliminated as soon as they appear (as part of the rule application) with the proviso that newly generated objects are replaced by older ones and not vice versa. In the following, $C$ and $D$ denote concepts, $\hat{R}$ a role, $f$ a feature, $P$ a predicate name from $\Phi_{\calD}$ with arity $n$, $u_1,u_2,\ldots,u_n$ feature chains, $a$ and $b$ objects for $O_A$ and $x_1,x_2,\ldots,x_n$ objects from $O_D$.
\begin{enumerate}
 \item $R\sqcap$, The \term{conjugate rule}. If $a:C\sqcap D\in\calA,\accol{a:C,a:D}\not\subseteq\calA$, then $\calA'=\calA\cup\accol{a:C,a;D}$.
 \item $R\sqcup$, The \term{(nondeterministic) disjunction rule}. If $a:C\sqcup D\in\calA,\accol{a:C,a:D}\cap\calA=\emptyset$, then $\calA'=\calA\cup\accol{a:C}\vee\calA'=\calA\cup\accol{a:D}$.
 \item $Rr\exists C$, The \term{role exists restriction rule}. If $a:\exists\hat{R}.C\in\calA,\not\exists b\in O_A:\accol{\tuple{a,b}:\hat{R},b:C}\subseteq\calA$, then $\calA'=\calA\cup\accol{\tuple{a,b}:\hat{R},b:C}$ where $b\in O_A$ is fresh in $\calA$.
 \item $Rf\exists C$, The \term{feature exists restriction rule}. If $a:\exists f.C\in\calA,\not\exists b\in O_A:\accol{\tuple{a,b}:f,b:C}\subseteq\calA$, then $\calA'=\calA\cup\accol{\tuple{a,b}:f,b:C}$ where $b\in O_A$ is fresh in $\calA$.
 \item $Rr\forall C$, The \term{role value restriction rule}. If $a:\forall\hat{R}.C\in\calA,\exists b\in O_A:\tuple{a,b}:\hat{R}\in\calA\wedge b:C\notin\calA$, then $\calA'=\calA\cup\accol{b:C}$
 \item $Rf\forall C$, The \term{feature value restriction rule}. If $a:\forall f.C\in\calA,\exists b\in O_A:\tuple{a,b}:f\in\calA\wedge b:C\notin\calA$, then $\calA'=\calA\cup\accol{b:C}$
 \item $R\exists P$, The \term{predicate exists restriction rule} (may create forks). If $a:\exists u_1,u_2,\ldots,u_n.P\in\calA,\not\exists x_1,x_2,\ldots,x_n\in O_D:\brak{\fun{\mbox{succ}_{\calA}}{a,u_1}=x_1\wedge\fun{\mbox{succ}_{\calA}}{a,u_2}=x_2\wedge\ldots\wedge\wedge\fun{\mbox{succ}_{\calA}}{a,u_n}=x_n\wedge\tuple{x_1,x_2,\ldots,x_n}:P\in\calA}$, then $\calC_0:=\calA\cup\accol{\tuple{x_1,x_2,\ldots,x_n}:P}$ where $x_1\in O_D$ are distinct and fresh in $\calA$. $\calC_1:=\fun{\mbox{chain}_{\calC_0}}{a,x_1,u_1},\calC_2:=\fun{\mbox{chain}_{\calC_1}}{a,x_2,u_2},\ldots,\calC_n:=\fun{\mbox{chain}_{\calC_{n-1}}}{a,x_n,u_n}$ and $\calA'=\displaystyle\cup_{i=0}^n\calC_i$
 \item $R\downarrow$, The \term{agreement rule} (may create forks). If $a:u_1\downarrow u_2\in\calA,\not\exists b\in O_A:\fun{\mbox{succ}_{\calA}}{a,u_1}=\fun{\mbox{succ}_{\calA}}{a,u_2}=b$ then $\calC=\calA\cup\fun{\mbox{chain}_{\calA}}{a,b,u_1}$ where $b\in O_A$ is fresh in $\calA$. $\calA'=\calC\cup\fun{\mbox{chain}_{\calC}}{a,b,u_2}$.
 \item $R\uparrow$, The \term{disagreement rule} (may create forks). If $a:u_1\uparrow u_2\in\calA,\not\exists b_1,b_2\in O_A:\brak{\fun{\mbox{succ}_{\calA}}{a,u_1}=b_1\wedge\fun{\mbox{succ}_{\calA}}{a,u_2}=b_2\wedge b_1\neq b_2\in\calA}$ then $\calC=\calA\cup\fun{\mbox{chain}_{\calA}}{a,b_1,u_1}$, $\calA'=\calC\cup\fun{\mbox{chain}_{\calC}}{a,b_2,u_2}\cup\accol{b_1\neq b_2}$ where $b_1b_2\in O_A$ are distinct and fresh in $\calA$.
\end{enumerate}
Rule applications that generate objects are called \term[Generating rule applications]{generating}. All other rule applications are called \term[Non-generating rule applications]{non-generating}. All applications of $Rr\exists C$ are generating. Applications of the rules $Rf\exists C$, $R\exists P$, $R\downarrow$, $R\uparrow$ are usually generating but may non-generating if fork elimination takes place.
\label{def:completionrulesabox}
\cite{conf/ijcai/Lutz99}
\end{defi}

\begin{defi}[Concrete domain satisfiable ABox, Contradictory ABox, Clash triggers for an ABox, Primitive clash for an ABox, Feature domain clash for an ABox, All domain clash for an ABox, Agreement clash for an ABox, Complete ABox]
Let the same naming conventions be given as in \defref{completionrulesabox}.. An ABox $\calA$ is called \term[Concrete domain satisfiable ABox]{concrete domain satisfiable} \iffTx{} \teTx{} a mapping $\delta$ from $O_D$ to $\Delta_{\calD}$ \stTx{} $\displaystyle\wedge_{\tuple{x_1,x_2,\ldots,x_n}:P\in\calA}\tuple{\fun{\delta}{x_1},\fun{\delta}{x_2},\ldots,\fun{\delta}{x_n}}\in P^{\calD}$ is true in $\calD$. An ABox $\calA$ is called \term[Contradictory ABox]{contradictory} if it is not concrete domain satisfiable or one of the following \term[Clash triggers for an ABox]{clash triggers} is applicable:
\begin{enumerate}
 \item \term[Primitive clash for an ABox]{Primitive clash}: $\accol{a:C,a:\neg C}\subseteq\calA$
 \item \term[Feature domain clash for an ABox]{Feature domain clash}: $\accol{\tuple{a,x}:f,\tuple{a,b}:f}\subseteq\calA$
 \item \term[All domain clash for an ABox]{All domain clash}: $\accol{\tuple{a,x}:f,a\forall f.C}\subseteq\calA$
 \item \term[Agreement clash for an ABox]{Agreement clash}: $a\neq a\in\calA$
\end{enumerate}
An ABox to which no completion rule is applicable is called \term[Complete ABox]{complete}.
\cite{conf/ijcai/Lutz99}
\end{defi}

\begin{defi}[Size of a concept, Size of an axiom, Size of an ABox]
...
\end{equation}
The \term{size of an axiom} $\alpha$ is \dabs{C} if $\alpha$ is of the form $x:C$ and 1 otherwise. The \term{size of an ABox} $\calA$ is the sum of the sizes of all axioms in $\calA$.
\cite{conf/ijcai/Lutz99}
\end{defi}
...
\begin{defi}[Selection function for a set of propositional sentences]
For any $K\in\calK$, a \term[Selection function for a set of propositional sentences]{selection function for $K$} is a function \funsig{S}{U_K}{2^{\calU_K}} \stTx{}
\begin{equation}
...
\begin{defi}[Partial meet contraction over a set of propositional sentences]
An operation \funsig{K\ominus}{\calF}{\calF} is a \term[Partial meet contraction over a set of propositional sentences]{partial meet contraction over $K$} \iffTx{} \teTx{} a selection function $S$ \stTx{} \fanTx{} $F\in\calF$,
\begin{equation}
...
\begin{defi}[Complete selection function for a set of propositional sentences, Complete partial meet contraction over a set of propositional sentences]
Let $K\in\calK$. A selection function $S$ for $K$ is \term[Complete selection function for a set of propositional sentences]{complete} if \faTx{} $H\in U_K$
\begin{equation}
\fun{S}{H}=\accol{K'\in H:\cap\fun{S}{H}\subseteq K'}
\end{equation}
A partial meet contraction function is \term[Complete partial meet contraction over a set of propositional sentences]{complete} if it can be generated from such a selection function.
\cite{conf/ijcai/ZhangCZC97}
\end{defi}

\begin{defi}[Rational selection function for a set of propositional sentences, Rational partial meet contraction over a set of propositional sentences, Transitive rational selection function for a set of propositional sentences, Transitive rational partial meet contraction over a set of propositional sentences]
Let $K\in\calK$. A selection function $S$ for $K$ is \term[Rational selection function for a set of propositional sentences]{rational} when \tiTx{} a relation $\leq$ on $\calU_K$ \stTx{} \fanTx{} $H\in U_K$,
\begin{equation}
\fun{S}{H}=\accol{X\in H:\forall Y\in\fun{H}{Y\leq X}}
\end{equation}
The meet contraction function generated from such $S$ is called a \term[Rational partial meet contraction over a set of propositional sentences]{rational partial meet contraction function}. If the relation $\leq$ is transitive, we cal $S$ a \term[Transitive rational selection function for a set of propositional sentences]{transitive rational selection function} and the meet contraction function generated from $S$ a \term[Transitive rational partial meet contraction over a set of propositional sentences]{transitive rational partial meet contraction}.
\cite{conf/ijcai/ZhangCZC97}
\end{defi}

\begin{defi}[Total-ordered partition of a belief set, Nice-ordered partition of a belief set]
For any belief set $K$, let $\calP$ be a partial of $K$ and $<$ a total order relation on $\calP$. The triple $\Sigma=\tuple{K,\calP,<}$ is called a \term[Total-ordered partition of a belief set]{Total-ordered partition of $K$} \tabbrev{Total-ordered partition of a belief set}{TOP}. For any $p\in\calP$, if $A\in p$, $p$ is called the rank of $A$, denoted by \fun{b}{A}. A total-ordered partition $\Sigma=\tuple{K,\calP,<}$ is a \term[Nice-ordered partition of a belief set]{nice-ordered partition of $K$} \tabbrev{Nice-ordered partition of a belief set}{NOP}, if it satisfies the following logical constraint:
\begin{equation}
\mbox{If }A_1,A_2,\ldots,A_n\vdash B\mbox{, then }\funm{sup}{\accol{\fun{b}{A_1},\fun{b}{A_2},\ldots,\fun{b}{A_n}}}\geq\fun{b}{B}
\end{equation}
\cite{conf/ijcai/ZhangCZC97}
\end{defi}

\begin{defi}[Nice-ordered partition contraction over a belief set]
Let $\Sigma=\tuple{K,\calP,<}$ be an nice-ordered partition of a belief set $K$. Define a function \funsig{\ominus}{\calF}{\calF}, called \termabbrev[Nice-ordered partition contraction over a belief set]{nice-ordered partition contraction}{NOP-contraction} over $K$, as follows; \faTx{} $F\in\calF$:
\begin{enumerate}
...
\begin{defi}[$\calA\calL\calC_{\calM}$-model based on a frame, Domain of a $\calA\calL\calC_{\calM}$-model based on a frame, Values of a concept in a world, Values of a role in a world, Truth relation in a $\calA\calL\calC_{\calM}$-model based on a frame, Satisfiable formula]
An \term{$\calA\cal\calL\calC_{\calM}$-model based on a frame} $\mathfrak{G}=\tuple{W,\vartriangleleft}^2$ is a pair $\mathfrak{M}=\tuple{\mathfrak{G},I}$, in which $I$ is a function associating with each $w\in W$ an $\calA\calL\calC$-model
\begin{equation}
\fun{I}{w}=\tuple{\Delta,R_0^{I,w},R_1^{I,w},\ldots,R_n^{I,w},C_0^{I,w},C_1^{I,w},\ldots,C_n^{I,w},a_0^{I,w},a_1^{I,w},\ldots,a_n^{I,w}}
\end{equation}
where $\Delta$ is a non-empty set. The \term[Domain of a $\calA\calL\calC_{\calM}$-model based on a frame]{domain} of $\calM$, $R_i^{I,w}$ are binary relations on $\Delta$, $C_i^{I,w}\subseteq\Delta$ and $a_i^{I,w}$ are objects in $\Delta$ such that $a_i^{I,u}=a_i^{I,v}$ \fanTx{} $u,v\in W$. The \tindex{Values of a concept in a world}\term[Values of a role in a world]{values $C^{I,w},R^{I,w}$ of a concept $C$ and a role $R$ in a world $w\in W$}, and the \term[Truth relation in a $\calA\calL\calC_{\calM}$-model based on a frame]{truth-relation $\brak{\calM,W}\vDash\varphi$} (or simply $w\vDash\varphi$) are defined inductively as follows:
\begin{enumerate}
 ...
 \item $w\vDash\neg\varphi$ \iffTx{} $w\not\vDash\varphi$;
\end{enumerate}
A formula $\varphi$ is \term[Satisfiable formula]{satisfiable} if \tiTx{} a model $\calM$ and a world $w$ in $\calM$ \stTx{} $w\vDash\varphi$.
\cite{conf/ijcai/WolterZ99}
\end{defi}

\begin{defi}[Concept type for a formula, Named concept type for a formula, Formula type for a formula, Named formula type for a formula, Type for a formula, Named type for a formula]
A \term[Concept type for a formula]{concept type for $\varphi$} is a subset of \funm{con}{\varphi} \stTx{}
\begin{enumerate}
 \item $C\wedge D\in t$ \iffTx{} $C,D\in t$, \feTx{} $C\wedge D\in\funm{con}{\varphi}$
 \item $\neg C\in t$ \iffTx{} $C\notin t$, \feTx{} $\neg C\in\fun{con}{\varphi}$
\end{enumerate}
A \term[Named concept type for a formula]{named concept type} is the pair $t_a=\tuple{t,a}$, where $t$ is a concept type and $a\in\funm{ob}{\varphi}$. A \term[Formula type for a formula]{formula type} and a \term[Named formula type for a formula]{named formula type} are defined analogously as saturated subsets of \funm{sub}{\varphi}. Finally, by a \term[Type for a formula]{type for $\varphi$} we mean the pair $\tau=\tuple{t,\Xi}$ where $t$ is the concept type and $\Xi$ a formula type for $\varphi$; $\tau_a=\tuple{t_a,\Xi_a}$ is a \term[Named type for a formula]{named type for $\varphi$}.
\cite{conf/ijcai/WolterZ99}
\end{defi}

\begin{defi}[Type tree for a formula]
By a \term[Type tree for a formula]{type tree for $\varphi$} we mean a structure of the form $\mathfrak{I}=\tuple{T,<}$ where $T$ is a finite set of labeled type for $\phi$ (so that one type may have many occurrences in $T$) and $<$ an intransitive tree order on $T$ \stTx{}
\begin{enumerate}
...
\begin{defi}[Type forest of a depth over a set of formulas]
A \term[Type forest of a depth over a set of formulas]{type forest of depth $d$ over a $\Delta$} is a set $\mathfrak{J}=\accol{\mathfrak{I}_x|x\in\Delta}$, where all $\calI_x=\tuple{T_x,<_x}$ are type trees for $\phi$ of the same depth $d$ and $\mathfrak{I}_a$, \feTx{} $a\in\Delta\cap\funm{ob}{\varphi}$, consists of only named types of the form $\tau_a$.
\cite{conf/ijcai/WolterZ99}
\end{defi}

% \begin{defi}[Run of a co-depth through a type forest]
% A \term[Run of a co-depth through a type forest]{run of co-depth $d$ through a type forest $\mathfrak{J}$ over $\Delta$} is a pair of the form
% \begin{equation}
% r=\tuple{\Delta_r,\accol{R_r|R\in\funm{rol}{\varphi}}}
% \end{equation}
% In which $\Delta_r$ contains precisely one type $\fun{r}{x}\in T_x$ of co-depth $d$ for every $x\in\Delta$ (so that $\Delta_r=\accol{\fun{r}{x}|x\in\Delta}$) and $R_r\subseteq\Delta_r\times\Delta_r$ \stTx{}:
% \begin{enumerate}
%  \item all types in $\Delta_r$ are formula equivalent to each other;
%  \item $\exists R.C\in\fun{r}{x}$ \iffTx{} $\exists y\in\Delta:\fun{r}{x} R_r\fun{r}{y}\wedge C\in\fun{r}{y}$ \feTx{} $\exists R.C\in\funm{con}{\varphi}$
%  %\item $C=D\in\fun{r}{x}$ \iffTx{} $\forall y\in\Delta:\fun{r}{x} R_r\fun{r}{y}\wedge C\in\fun{r}{y}$ \feTx{} $\exists R.C\in\funm{con}{\varphi}$
% \end{enumerate}
% \cite{conf/ijcai/WolterZ99}
% \end{defi}
