...
\section{Wat zijn Constraint Problems?}
...
% \section{Leidend Voorbeeld: 4-Houses Puzzle}
% Als leidend voorbeeld zullen we de 4-Houses Puzzle oplossen. Deze puzzel stelt een straat waarbij 4 families naast elkaar wonen: $A$, $B$, $C$ en $D$. Deze huizen zijn genummerd van $1$ tot $4$, en liggen respectievelijk naast elkaar. Het doel van de puzzel is bij iedere familie het juiste huisnummer te plaatsen. Hiervoor worden enkele tips gegeven:
% \begin{itemize}
%  \item $D$ woont in een huis met een lager huisnummer dan $B$
%  \item $B$ woont naast $A$ met een hoger huisnummer
%  \item Er ligt tenminste 1 huis tussen $B$ en $C$
%  \item $D$ woont niet in een huis met nummer 2
%  \item $C$ woont niet in een huis met nummer 4
% \end{itemize}
% \paragraph{}
% Indien we de bovenstaande situatie nu formaliseren stellen we 4 variabelen: $A$, $B$, $C$ en $D$. Iedere variabele heeft een initieel domein $d_A=d_B=d_C=d_D=\left\{1,2,3,4\right\}$. We beschikken over volgende unaire constraints:
% \begin{equation}
% \left\{\begin{array}{l}
% c\left(C\right):=C\neq 4\\
% c\left(D\right):=D\neq 2
% \end{array}\right.
% \end{equation}
% En als binaire constraints stellen we:
% \begin{equation}
% \left\{\begin{array}{l}
% c\left(A,B\right):=A=B-1\\
% c\left(A,C\right):=A\neq C\\
% c\left(A,D\right):=A\neq D\\
% c\left(B,C\right):=\left|B-C\right|\geq 2\\
% c\left(B,D\right):=B>D\\
% c\left(C,D\right):=C\neq D\\
% \end{array}\right.
% \end{equation}
\section{Leidend Voorbeeld: 4-Teachers}
\begin{leftbar}
...
% \begin{equation}
% \begin{array}{lclcl}
% d_A=\left\{1,2,3,4\right\}&\wedge&c\left(A\right):=\true&\Rightarrow&d_A'=\left\{1,2,3,4\right\}\\
% d_B=\left\{1,2,3,4\right\}&\wedge&c\left(B\right):=\true&\Rightarrow&d_B'=\left\{1,2,3,4\right\}\\
% d_C=\left\{1,2,3,4\right\}&\wedge&c\left(C\right):=C\neq4&\Rightarrow&d_C'=\left\{1,2,3\right\}\\
% d_D=\left\{1,2,3,4\right\}&\wedge&c\left(D\right):=D\neq2&\Rightarrow&d_D'=\left\{1,3,4\right\}
% \end{array}
% \end{equation}
\begin{leftbar}
Toegepast op ons voorbeeld genereren we dus een nieuw domein voor iedere variable met:
...
\end{equation}
\end{leftbar}
\section{Visuele voorstelling van het probleem}
...
\end{figure}
\subsection{Het Constraint Network}
...
\caption{$\mathcommand{chronologicalBacktracking}{\depth}$}          % give the algorithm a caption
\label{alg:chronologicalBacktracking}                           % and a label for \ref{} commands later in the document
...
\begin{algorithm}[htb]                      % enter the algorithm environment
\caption{$\mathcommand{backjumping}{\depth,\out l}$}          % give the algorithm a caption
\label{alg:backjumping}                           % and a label for \ref{} commands later in the document
\begin{algorithmic}[1]                    % enter the algorithmic environment
\STATE $l\leftarrow0$
\FOR{$k=1$ to $n_{\depth}$}
\STATE\COMMENT{Iteratie over alle knopen op niveau $\depth$}
...
\begin{algorithm}[htb]                      % enter the algorithm environment
\caption{$\mathcommand{backmarking}{\depth}$}          % give the algorithm a caption
\label{alg:backmarking}                           % and a label for \ref{} commands later in the document
\begin{algorithmic}[1]                    % enter the algorithmic environment
\FOR{$k=1$ to $n_{\depth}$}
...
\section{Relaxatie en Hybrid Constraint Processing}
...
\end{figure}
\subsection{Arc Consistency Technieken}
...
\label{alg:arcConsistency3}                           % and a label for \ref{} commands later in the document
\begin{algorithmic}[1]                    % enter the algorithmic environment
...
\section{Toepassingen en Alternatieven}
