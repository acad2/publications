...
\begin{defi}[Convex subset]
A subset $C\subseteq CDL$ is said to be \term[Convex subset]{convex} \iffTx{} \faTx{} $c_1,c_2\in C$, $c_3\in CDL$, $c_1\preceq c_3\preceq c_2$ implies $c_3\in C$
\cite{conf/ijcai/Hirsh91}
\end{defi}

\begin{defi}[Set of minimal elements of a set, Set of maximal elements of a set]
The \term[Set of minimal elements of a set]{set of minimal elements of $C$} is written \funm{Min}{C}, and is defined by
\begin{equation}
\funm{Min}{C}=\accol{c\in C|\neg\exists c'\in C\mbox{ \stTx{} }c'\prec c}
\end{equation}
Similarly, the \term[Set of maximal elements of a set]{set of maximal elements of $C$}, \funm{Max}{C} is
\begin{equation}
...
\begin{defi}[Definite set]
$C$ is \term[Definite set]{definite} \iffTx{} \faTx{} $c\in C$ there exists some $s\in\funm{Min}{C}$ and some $g\in\funm{Max}{C}$ \stTx{} $s\preceq c\preceq g$
\cite{conf/ijcai/Hirsh91}
\end{defi}

\begin{defi}[Set representable by boundary sets]
If $C\subseteq CDL$, $S=\funm{Min}{C}$, and $G=\funm{Max}{C}$, $C$ is said to be \term[Set representable by boundary sets]{representable by boundary sets} \iffTx{}
\begin{equation}
...
\begin{defi}[Believe in absolute independent events]
\term[Believe in absolute independence]{Believing $C$ is absolutely independent of $A$} (noted $A\perp_{>} C$) \iffTx{} $\fun{N}{C|A}=\fun{N}>0$
\cite{conf/ijcai/DuboisCHP97}
\end{defi}

\begin{defi}[Not informing another event]
$A$ does \term[Not informing another variable]{not inform about $C$} \iffTx{} $\fun{N}{C|A}=\fun{N}{C}=\fun{N}{\neg C|A}=\fun{N}{\neg C}=0$.
\cite{conf/ijcai/DuboisCHP97}
\end{defi}

\begin{defi}[Believe in qualitative independent events]
\term[Believe in qualitative independence]{Believing $C$ is qualitatively independent of $A$} (denoted $A\neq_{>}C$) \iffTx{} $\fun{N}{C}>0$ and $\fun{N}{C|A}>0$.
\cite{conf/ijcai/DuboisCHP97}
\end{defi}

\begin{defi}[Qualitatively relevant events to believe]
$A$ is \term[Qualitatively relevant to believe]{qualitatively relevant to believing} $C$. denoted ($A=_{>}C$), \iffTx{} $\fun{N}{C}>0$ and $\fun{N}{C|A}=0$.
\cite{conf/ijcai/DuboisCHP97}
\end{defi}

\begin{defi}[Event justifies another event]
$A$ \term[Event justifies another event]{justifies} $C$ \iffTx{} $\fun{N}{C}=\fun{N}{\neg C}$ and $\fun{N}{C|A}>0$.
\cite{conf/ijcai/DuboisCHP97}
\end{defi}