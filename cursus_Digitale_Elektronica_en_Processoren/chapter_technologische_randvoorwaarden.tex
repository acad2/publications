...
\label{fig:dynamicBehaviorNotGate}
\end{figure}
...
\draw (-2.6,1) |- (0,-1) -- (TG.c);
\end{tikzpicture}
\label{fig:triStateImplementation}}
...
\draw (P.source) -| (0.5,0) |- (N.source);
\draw (P.drain) -| (-0.5,0) |- (N.drain);
\draw (-0.5,0) -- ++(-0.5,0) node[anchor=east]{$x$};
\draw (0.5,0) -- ++(0.5,0) node[anchor=west]{$f$};
\draw (P.gate) -- ++(0,0.5) node[anchor=south]{$s$};
\draw (N.gate) -- ++(0,-0.5) node[anchor=north]{$s$};
\end{scope}
\end{tikzpicture}
\label{fig:triStateTransmission}}
\caption{Tri-state buffer.}
\end{figure}
Tri-state buffers worden vaak gebruikt om bijvoorbeeld over een lijn gegevens van verschillende bronnen over te brengen. In dat geval moeten de andere bronnen een $Z$ op de lijn zetten.
