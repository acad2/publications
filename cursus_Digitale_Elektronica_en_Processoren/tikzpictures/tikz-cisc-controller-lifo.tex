...
\node[subcomp,minimum width=1.1 cm,minimum height=0.25] (INC) at ($(M.data1)+(-1.25,-0.25)$) {$+1$};
%\coordinate (LFD) at (M.data0 -| INC.north);
...
\coordinate (CSD) at ($0.333*(NSOL.south east)+0.667*(NSOL.east)$);\coordinate (CSE) at ($0.75*(NSOL.south east)+0.25*(NSOL.east)+(0.75,0)$);\coordinate (CSF) at ($(CS)+(0,-0.25)$);\draw[thick] (CSD) -- (CSE |- CSD) |- (CSF) -- (CS);
