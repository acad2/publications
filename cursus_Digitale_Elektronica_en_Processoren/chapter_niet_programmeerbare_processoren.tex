...
\begin{enumerate}
 ...
 \item In een registerbank is het signaal ``read-enabled (RE)'' altijd gelijk aan het stuursignaal op het tri-state buffer (RFOE). We kunnen dus voor beide leespoorten ofwel \mbox{RE$i$} ofwel \mbox{RFOE$i$}. We besparen twee bits (we beschouwen immers twee leespoorten).
 \item De resultaatbus bevat het resultaat van ofwel de ALU ofwel de barrel shifter. 
\end{enumerate}
