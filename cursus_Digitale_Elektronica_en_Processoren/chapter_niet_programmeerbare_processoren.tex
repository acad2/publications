...
\begin{figure}[hbt]
\centering
\begin{sprocessor}{4}{x/$X$/1/2,y/$Y$/3/2,z/$Z$/3/2,i/$I$/1/2}{5}{sa/{$\shlcmd 1$},sb/{$\shlcmd 1$},rcc/$y_2y_1y_01$,rca/$z_2z_1z_0x_3$,rcb/{$z_2z_1z_0x_3$\\$-D$},de/$-1$}
\node[anchor=south] (VN) at ($(RRx-1)+(0,0.5)$) {$N$};
\node[anchor=south] (V3) at (RRi-1 |- VN.south) {$3$};
\coordinate (VDC) at ($(FTrcb)+(-0.3,0)$);
...
\sprbtf{x}{sa}{0};
\end{sprocessor}
\caption{Implementatie van het leidend voorbeeld via de basisprincipes.}
\end{figure}
