...
What is left to prove, is that each $n$-by-$n$ \concept{interval matrix} $M$ for which $det(M)\neq 0$ is \concept{unimodular}, which means it has a \concept{determinant} of $+1$ or $-1$. We will do this by showing that we can transform $M$ into an \concept{identity matrix} with the \concept{row} operations defined above.\\
The first step in this proof is to define for each \concept{row} $i \in 1..n$ the variables $s_i$ and $e_i$, denoting respectively the \concept{column} where the \concept{interval} of $1$'s starts and ends. For instance, the following \concept{interval matrix} has three \concepts{row}, where $s_1=1$, $s_2=1$, $s_3=2$, $e_1=3$, $e_2=2$, $e_3=3$:
\begin{equation}
...
\end{array}}
\end{equation}
Now, whenever two rows $i$ and $j$ have $s_i=s_j$ or $e_i=e_j$, we subtract the row with the fewest $1$'s from the \concept{row} with the most $1$'s. This transformation preserves both the consecutive ones property and the \concept{unimodular} property, so we end up with a new \concept{interval matrix} $M'$ with the same \concept{unimodular} property\footnote{This transformation can change the sign of the \concept{determinant} though.}, but with less $1$'s. For example, \concept{matrix} \ref{unimodular_interval} can be transformed into
\begin{equation}
\brak{\begin{array}{ccc}
0&0&1\\
1&1&0\\
...
\end{array}}
\end{equation}
by noting that $s_1=s_2$, and subsequently subtracting the \concept{row} $2$ from the \concept{row} $1$.\\
If we now continue to apply this \concept{row} operation until no two \concepts{row} have the same start value or end value, we end up with a \concept{matrix} containing exactly one $1$ in each \concept{row}, never in the same \concept{column}.\footnote{Note that no empty rows or columns can be derived, since we could assume the determinant was non-zero.} One way to understand this, is by noting that the $n$ $s_i$'s of the final \concept{matrix} must all be different, and since they can only take the values $1..n$ (being the \concept{column} numbers), each \concept{column} is associated to exactly one $s_i$. The same argument can be made for the $e_i$'s. Now, since no $e_i$ can be smaller than its corresponding $s_i$, $e_i=s_i$ for all $i \in 1..n$. Continuing the example yields \ref{unimodular_interval_2} after subtracting \concept{row} $1$ from \concept{row} $3$ and then \concept{row} $3$ from \concept{row} $2$:
\begin{equation}
...
\end{array}}
\end{equation}
Such a \concept{matrix} can after some row-swap operations be transformed into the identity, e.g.:
\begin{equation}
\brak{\begin{array}{ccc}
1&0&0\\
0&1&0\\
...
\end{array}}
\end{equation}
And since the identity is \concept{unimodular}, and since we only used unimodularity preserving row operations, $M$ must be \concept{unimodular}, which concludes the proof.
\end{answer}
\end{exercise}
\begin{exercise}
Consider \excref{rectanglestabbing} of \chpref{formulations}, and let us modify that problem by only considering \concepts{vertical line} as potential stabbing lines. The resulting problem can be formulated as follows: given a set of \concepts{interval}, find a minimum number of \concepts{vertical line} stabbing each interval at least once.
\begin{enumerate}
 \item Give an \concept{IP-formulation} of the resulting problem.
 \item Is the corresponding constraint \concept{matrix} an \concept{interval matrix} (see \excref{interval-matrix})?
 \item How would you solve this problem?
 \item Can you use the same approach for the \concept{rectangle stabbing problem}?
\end{enumerate}
\end{exercise}
\begin{answer}
The \concept{IP-formulation} is the following:
\begin{eqnarray}
...
\end{eqnarray}
The corresponding \concept{matrix} is \concept{TUM} since it is an \concept{interval matrix}. You can solve this problem using \concept{linear programming} since the \concept{matrix} is \concept{TUM} and thus the variables are guaranteed to be integral. It is not possible to solve the \concept{rectangle stabbing problem} since such \concept{matrix} is not guaranteed to be \concept{TUM}.
\end{answer}
\begin{exercise}
...
\end{array}
\end{equation}
Solving the \concept{linear program} by the \concept{simplex method} gives the following final \concept{dictionary}.
\begin{equation}
\begin{array}{rcrcrcr}
...
\end{equation}
\paragraph{}
Find two \concepts{Gomory cut} for the problem where the \concepts{variable} $x_1$ and $x_2$ need to be \concept[integral variable]{integral}. Next, rewrite each of these \concept[Gomory cut]{cuts} in terms of solely $x_1$ and $x_2$.
\end{exercise}
\begin{exercise}
Consider the following problem in \concepts{integral variable} $x_1$ and $x_2$. How many \concepts{cutting plane} do you have to add to solve the problem for $k=2$? For $k=4$? For general $k$? Can you use integer rounding to generate a \concept{valid inequality}?
\begin{hint}
Draw a picture.
\end{hint}
\begin{equation}
...
\end{exercise}
\begin{exercise}
Let $G=\tupl{V,E}$ be an \concept{undirected graph}. A \concept{matching} in $G$ is represented by a \concept{vector} $\vec{x}\in\BoolSet^{\abs{E}}$ in the following way. For each \concept{edge} $e\in E$ we define

\begin{equation}
\semboolvar{x_e}{if edge $e$ is selected in the matching;}{otherwise.}
\end{equation}

The set of \concepts{matching} is now given by
...
\begin{enumerate}
 \item Derive with \concept{integer rounding} that
\begin{equation}
\sumdomain[e=\tupl{v_1,v_2}]{E:v_1,v_2\in S}{x_e}\leq\displaystyle\frac{\abs{S}-1}{2}
\eqnlab{matching-introunding}
\end{equation}
is a \concept{valid inequality} for each $S\subseteq V$ with $3\leq\abs{S}\leq\abs{V}$ and $\abs{S}$ odd. (These inequalities are called \concepts{odd-set constraint}).
 \item Find, for the \concept{graph} depicted in \figref{triangle}, a fractional solution violating the inequality \eqnref{matching-introunding} with $S=\accl{1,2,3}$.
\end{enumerate}
\importtikzfigure{triangle}{A triangle.}
\end{exercise}
\begin{exercise}
Consider the \concept{integer programming formulation} of the \concept{clique partitioning problem} (see \excref{clique-partitioning} of \chpref{formulations}). \figref{cliquepartition-integer} shows an instance of that problem. Notice that solid lines correspond to \concepts{edge} with \concept{weight} $+1$, whereas dotted lines correspond to \concepts{edge} with \concept{weight} $-1$. What is the value of the \concept{linear programming relaxation} of the \concept{integer programming formulation} corresponding to this instance? Can you find a violated inequality using \concept{integer rounding}?
