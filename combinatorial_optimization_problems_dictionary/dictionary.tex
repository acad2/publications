...
\dey[collapsing knapsack]{Collapsing knapsack}{A family of problems that generalize \rey{knapsack} problems. In the \rey{knapsack} problem the \nmn{capacity}\footnote{Or capacities.} of the knapsack is fixed whereas in the \todef{} problem, the capacity is determined by a non-increasing function in the number of items included. Like the \rey{knapsack} family, several flavors of the problem exists.}[0-1 collapsing knapsack]

\dey[0-1 collapsing knapsack]{0-1 collapsing knapsack}{\frml{\fcop{\sumieqb{1}{n}c_i\cdot x_i}{\sumieqb{1}{n}{a_i\cdot x_i}\leq\fun{b}{\sumieqb{1}{n}{x_i}}\\&x_i\in\accl{0,1}&\iran{1}{n}}}The \todef{} has applications in \nmn{satellite communication} and \nmn{scheduling}. In satellite communication, transmissions on the band require gaps between the portions of the band assigned to each user. On \nmn{time-sharing computer systems} each processes that runs on the system causes additional overhead thus reducing the capacity.}
