...
\dey[frequency assignment]{Frequency assignment}{A family of problems related to wireless communication. In all problems a set of \nmns{transmitter} is given and each of these transmitters must be assigned one or more \nmns{frequency}. Transmitters closely to each other can't use the same frequencies or frequencies close to each other since this results in inference. Several problems aim to minimize different aspects of the problem like the total inference, the number of used frequencies,...}[minimum order frequency assignment,minimum span frequency assignment,minimum blocking frequency assignment,minimum inference frequency assignment]

\dey[minimum order frequency assignment]{Minimum order frequency assignment}{Given an \nmn{undirected graph} $G=\tupl{V,E}$ of \nmns{transmitter} $V$ such that for each $v\in V$, $\tupl{v,v}\notin E$. Each of the transmitters $v$ is annotated with a set of possible frequencies $D_v$. Each of the edges $\tupl{v,w}$ is annotated with a set of unacceptable inferences $T_{v\;w}$. The objective is to assign a set of frequencies $f_v$ to each of the transmitters $v$ such that each submitter is assigned a fixed number of frequencies $c_v$, no two transmitters $\tupl{v,w}\in E$ have a bandwidth that differs with a unacceptable inference $T_{v\;w}$ and the total number of used bandwidth is minimized.\frml{\fcop[-]{\abs{\cupdomain[v]{V}{f_v}}}{\abs{f_v}=c_v&v\in V\\&f_v\subseteq D_v&v\in V\\&\abs{f_{v\;i}-f_{w\;j}}\notin T_{v\;w}&\tupl{v,w}\in E,f_{v\;i}\in f_v,f_{w\;j}\in f_w}}}

\dey[minimum span frequency assignment]{Minimum span frequency assignment}{Given an \nmn{undirected graph} $G=\tupl{V,E}$ of \nmns{transmitter} $V$ such that for each $v\in V$, $\tupl{v,v}\notin E$. Each of the transmitters $v$ is annotated with a set of possible frequencies $D_v$. Each of the edges $\tupl{v,w}$ is annotated with a set of unacceptable inferences $T_{v\;w}$. The objective is to assign a set of frequencies $f_v$ to each of the transmitters $v$ such that each submitter is assigned a fixed number of frequencies $c_v$, no two transmitters $\tupl{v,w}\in E$ have a bandwidth that differs with a unacceptable inference $T_{v\;w}$ and the span between the lowest and highest assigned frequency is minimized.\frml{\fcop[-]{\max{\cupdomain[v]{V}{f_v}}-\min{\cupdomain[v]{V}{f_v}}}{\abs{f_v}=c_v&v\in V\\&f_v\subseteq D_v&v\in V\\&\abs{f_{v\;i}-f_{w\;j}}\notin T_{v\;w}&\tupl{v,w}\in E,f_{v\;i}\in f_v,f_{w\;j}\in f_w}}}

\dey[minimum blocking frequency assignment]{Minimum blocking frequency assignment}{Given an \nmn{undirected graph} $G=\tupl{V,E}$ of \nmns{transmitter} $V$ such that for each $v\in V$, $\tupl{v,v}\notin E$. Each of the transmitters $v$ is annotated with a set of possible frequencies $D_v$ and a non-increasing \nmn{blocking function} $\funsig{b_v}{\NNN_0}{\NNN_0}$. Each of the edges $\tupl{v,w}$ is annotated with a set of unacceptable inferences $T_{v\;w}$. The objective is to assign a set of frequencies $f_v$ to each of the transmitters $v$ such that each submitter is assigned at most a fixed number of frequencies $c_v$, no two transmitters $\tupl{v,w}\in E$ have a bandwidth that differs with a unacceptable inference $T_{v\;w}$ and the sum of the blocking functions for the number of frequencies assigned to each transmitter is minimized.\frml{\fcop[-]{\sumdomain[v]{V}{\fun{b_v}{\abs{f_v}}}}{\abs{f_v}\leq c_v&v\in V\\&f_v\subseteq D_v&v\in V\\&\abs{f_{v\;i}-f_{w\;j}}\notin T_{v\;w}&\tupl{v,w}\in E,f_{v\;i}\in f_v,f_{w\;j}\in f_w}}}

\dey[minimum inference frequency assignment]{Minimum inference frequency assignment}{Given an \nmn{undirected graph} $G=\tupl{V,E}$ of \nmns{transmitter} $V$ such that for each $v\in V$, $\tupl{v,v}\notin E$. Each of the transmitters $v$ is annotated with a set of possible frequencies $D_v$ and a penalty $p_{v\;w\;f_{v\;i}\;f_{w\;j}}$ for each two transmitters and their possible frequencies $f_{v\;i}\in D_v$ and $f_{w\;j}\in D_w$. Each of the edges $\tupl{v,w}$ is annotated with a set of unacceptable inferences $T_{v\;w}$. The objective is to assign a set of frequencies $f_v$ to each of the transmitters $v$ such that each submitter is assigned a fixed number of frequencies $c_v$ and the sum of the penalties for assigning unacceptable inference frequencies $T_{v\;w}$ to two transmitters $v$ and $w$ is minimized.\frml{\fcop[-]{\sumdomain[\tupl{v,w}]{E}{\sumdomain[f_{v\;i}]{f_v}{\sumdomain[f_{w\;j}]{f_w}{p_{v\;w\;f_{v\;i}\;f_{w\;j}}\cdot\kronecker{\abs{f_{v\;i}-f_{w\;j}}\in T_{v\;w}}}}}}{\abs{f_v}=c_v&v\in V\\&f_v\subseteq D_v&v\in V}}Sometimes the list of penalties is not given explicitly but calculated based on a function where the distance between the transmitters is taken into account.}

\dey[collapsing knapsack]{Collapsing knapsack}{A family of problems that generalize \rey{knapsack} problems. In the \rey{knapsack} problem the \nmn{capacity}\footnote{Or capacities.} of the knapsack is fixed whereas in the \todef{} problem, the capacity is determined by a non-increasing function $\funsig{b}{\NNN}{\NNN}$ in the number of items included. In case the function is linear, the problem can be converted to a \rey{knapsack} problem. Like the \rey{knapsack} family, several flavors of the problem exists.}[0-1 collapsing knapsack]

\dey[0-1 collapsing knapsack]{0-1 collapsing knapsack}{A special form of the \rey{collapsing knapsack} problem. In this variant, a bag contains zero or one instances of every item, in other words no copies of the specified items are allowed. \frml{\fcop{\sumieqb{1}{n}c_i\cdot x_i}{\sumieqb{1}{n}{a_i\cdot x_i}\leq\fun{b}{\sumieqb{1}{n}{x_i}}\\&x_i\in\accl{0,1}&\iran{1}{n}}}The \todef{} has applications in \nmn{satellite communication} and \nmn{scheduling}. In satellite communication, transmissions on the band require gaps between the portions of the band assigned to each user. On \nmn{time-sharing computer systems} each processes that runs on the system causes additional overhead thus reducing the capacity.}

\dey[expanding knapsack]{expanding knapsack}{A family of problems that generalize \rey{knapsack} problems. In the \rey{knapsack} problem the \nmn{capacity}\footnote{Or capacities.} of the knapsack is fixed whereas in the \todef{} problem, the capacity is determined by a nondecreasing function $\funsig{b}{\NNN}{\NNN}$ in the number of items included. In case the function is linear, the problem can be converted to a \rey{knapsack} problem. Like the \rey{knapsack} family, several flavors of the problem exists.}[0-1 expanding knapsack]

\dey[job shop scheduling]{Job shop scheduling}{Given a set of $n$ \nmns{job} $J_i$ and a set of $m$ \nmns{machine} $M_r$. Each job $J_i$ has a list of \nmns{operation} $O_{i\;j}$ that must be processed on a specific machine with a \nmn{duration} of $d_{i\;j}$. A machine can only handle one operation at a time and operations must be processed atomically and in the order specified by the corresponding job. Operations can however be processed in parallel if they require a different machine. The aim is to minimize the \nmn{makespan}: the time it takes to process all operations.}

\dey[scheduling]{Scheduling}{\todef{} is a family of problems where each problems aims to allocate shared resources over time to competing activities.}[job shop scheduling]
...
\dey[0-1 expanding knapsack]{0-1 expanding knapsack}{}
\dey[cutting stock]{Cutting stock}{}
\dey[multiperiod knapsack]{Multiperiod knapsack}{}
\dey[nested knapsack]{Nested knapsack}{}
\dey[0-1 multiple choice nested knapsack]{0-1 multiple choice nested knapsack}{\frml{\fcop{\sumieqb{1}{m}{v_i\cdot x_{i\;1\;1}-\sumieqb[j]{1}{m}{c_s\cdot y_js}}}{
\sumieqb{1}{m}{l_i\cdot x_{i\;1\;1}}\leq L_1\\
&\sumieqb{1}{m}{l_i\cdot x_{i\;j\;s}}\leq L_s\cdot y_{j\;s}&\iran[j]{1}{m},\iran[s]{2}{S}\\
&x_{i\;1\;1}\leq\sumieqb[j]{1}{m}{x_{i\;j\;s}}&\iran[j]{1}{m},\iran[s]{1}{S}\\
&\maxv[j]{x_{p\;j\;s}+x_{q\;j\;s}}\leq&\maxv[j]{x_{p\;j\;s-1}+x_{q\;j\;s-1}}&\iran[p,q]{1}{m},\iran[s]{3}{S},p\neq q\\
&x_{i\;j\;s}\in\BBB&\iran[i,j]{1}{m},\iran[s]{1}{S}\\
&y_{j\;s}\in\BBB&\iran[j]{1}{m},\iran[s]{1}{S}
}}}
\dey[flow shop sequencing]{Flow shop sequencing}{}
\dey[open shop scheduling]{Open shop scheduling}{}
\dey[quadratic assignment]{Quadratic assignment}{}
\dey[vehicle routing]{Vehicle routing}{}
\dey[capacitated vehicle routing]{Capacitated Vehicle routing}{}
\dey[heterogeneous fleet vehicle routing]{Heterogeneous fleet vehicle routing}{}
...
\aey[job-shop]{Job-shop}{job shop scheduling}

%abbreviations
\bey{MO-FAP}{minimum order frequency assignment}
\bey{MS-FAP}{minimum span frequency assignment}
\bey{MB-FAP}{minimum blocking frequency assignment}
\bey{MI-FAP}{minimum inference frequency assignment}
\bey{CKP}{collapsing knapsack}
\bey{JSSP}{job shop scheduling}
\bey{KP}{knapsack}
\bey{BPP}{bin-packing}
\bey{NKP}{nested knapsack}
